...
class SavingsGoal
{
    ...
    class FinanaceManager
    {
        ...
        private decimal budgetThreshold;

        public event EventHandler<decimal> BudgetExceeded;

        public FinanaceManager(decimal threshold = 1000)
        {
            transactions = new List<Transaction>();
            savingsGoals = new List<SavingsGoal>();
            budgetThreshold = threshold;
        }
        //Add transaction with event triggering
        public void AddTransaction(Transaction transaction)
        {
            ...
            {
                decimal monthlyExpenses = GetMonthlyExpenses();
                if (monthlyExpenses > budgetThreshold)
                {
                    OnBudgetExceeded(monthlyExpenses);
                }
            }
        }

        protected virtual void OnBudgetExceeded(decimal amount)
        {
            BudgetExceeded?.Invoke(this, amount);
        }
